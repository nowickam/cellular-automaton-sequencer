...
void Bela_listHook(const char *source, int argc, t_atom *argv)
{
    ...
    if (0 == strcmp(source, "bela_guiOut"))
    {
        ...
        { // otherwise we send each element of the list separately
            for (int n = 1; n < argc; ++n)
            {
                t_atom *a = &argv[n];
                if (libpd_is_float(a))
...
void Bela_messageHook(const char *source, const char *symbol, int argc, t_atom *argv)
{
    ...
    if (0 == strcmp(source, "bela_setGui"))
    {
        ...
        {
            if (
                argc < 2 || !libpd_is_symbol(argv) || !libpd_is_symbol(argv + 1))
            {
                return;
            }
    ...
    if (0 == strcmp(source, "bela_setTrill"))
    {
        ...
        {
            if (argc < 2 || !libpd_is_symbol(argv) || !libpd_is_symbol(argv + 1))
            {
                setTrillPrintError();
                return;
            }
        ...
        if (
            0 == strcmp(symbol, "threshold") || 0 == strcmp(symbol, "prescaler"))
        {
            if (
                argc < 2 || !libpd_is_symbol(argv) || !libpd_is_float(argv + 1))
            {
                setTrillPrintError();
                return;
            }
